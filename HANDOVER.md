...
**UX breakdown tracker = `.redesign/UX_FLOWS.md`** (per-journey MAP→FRICTION→UX→UI→COPY checklist). Done: J1, J2, J4, **J8**, **J5** (link-out tier). **Pending:** J6 (store — POD fulfilment + webhook persistence + real imagery), J3 (create), J9 (resolution→NFT certificate), J7 (dashboard), J10 (treasury), J11 (info). Method: dissect ONE journey at a time; present grounded MAP + friction; get UX forks via questions; then UI; then COPY; commit per journey. **Natural next: J6 (store)** — half-built, fulfilment + webhook persistence + imagery TODO.
**J5 note:** shipped the BACK action as a **link-out to bags.fm** (`bags.fm/<tokenMint>` on the dossier when `live`). `quote`/`swap` APIs still unwired — an **in-app custodial swap via qubik wallet** is the deferred next tier; also deferred: market-row quick-back. Decisions: anyone backs / residents vote.
