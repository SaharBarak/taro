...
**UX breakdown tracker = `.redesign/UX_FLOWS.md`** (per-journey MAP→FRICTION→UX→UI→COPY checklist). Done: J1, J2, J4, **J8**, **J5** (link-out tier), **J6** (integrity+imagery tier). **Pending:** J3 (create), J9 (resolution→NFT certificate), J7 (dashboard), J10 (treasury), J11 (info). Method: dissect ONE journey at a time; present grounded MAP + friction; get UX forks via questions; then UI; then COPY; commit per journey. **Natural next: J3 (create)** — narrower persona; or J9 (close the post-vote loop).
