...
**UX breakdown tracker = `.redesign/UX_FLOWS.md`** (per-journey MAP→FRICTION→UX→UI→COPY checklist). Done: J1, J2, J4, **J8**, **J5** (link-out tier), **J6** (integrity+imagery tier), **J9** (certificate view-only tier), **J3** (create finalised + gated). **Pending:** J7 (dashboard pass), J10 (treasury), J11 (info) — the remaining lower-priority/retention journeys. Method: dissect ONE journey at a time; present grounded MAP + friction; get UX forks via questions; then UI; then COPY; commit per journey. **Natural next: J7 (dashboard) + J10 (treasury)** — retention + trust.
**J3 note:** create now finalises via `/he/payments/return` (reads stashed draft → `POST /api/votes`, retries webhook lag). Gate = fully verified resident, vote always for the creator's own municipality (server-derived). Price flipped to ₪50.
