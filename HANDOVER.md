...
3. **Deferred per-journey infra (STACK LOCKED — see [[taruu-stack-decisions]] / `DNS-SETUP.md`):** Solana, not qubik. **Remaining:** J5 = backing/swap via Jupiter; J6 = POD fulfilment webhook (Printful/Printify — TBC); J7-push = push notifications (Expo SDK already a dep). **DONE this session:** ~~J4 OTP~~ (Workers KV, Twilio removed); ~~J7 refund~~ (request flow: `/api/payments/refund` + `paddleService.createRefund` + support intake); ~~J9 NFT mint~~ (Solana cNFT via Metaplex Bubblegum + Pinata IPFS + Helius, `/api/cron/mint-nfts` batch minter — needs a mainnet smoke test with live creds + a Bubblegum merkle tree).
...
4. **BAGS trading unwired:** `/api/bags/quote` + `/api/bags/swap` exist; no buy/back UI or wallet connect (J5). Stack: Solana + Jupiter (see [[taruu-stack-decisions]]).
