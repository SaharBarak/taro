...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const facebookUser = await getFacebookUserInfo(longLivedTokens.access_token);
    ...
    // Upsert Facebook social proof in Supabase
    await upsertSocialProof({
      user_id: user.id,
      provider: 'facebook',
      provider_id: facebookUser.id,
      provider_name: facebookUser.name,
      provider_email: facebookUser.email || null,
    ...
    return NextResponse.redirect(
      new URL(
        '/settings/social-connections?success=facebook',
        process.env.NEXT_PUBLIC_APP_URL
      )
    );
  } catch (error) {
